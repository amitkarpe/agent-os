...
class InventoryTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        (self.root / "unreferenced.zip").write_bytes(b"zip")
        (self.root / "RESULT.md").write_text("Selected proof: [capture.zip](capture.zip)\n", encoding="utf-8")
        (self.root / "pointers").mkdir()
        (self.root / "pointers" / "source.txt").write_text("source\n", encoding="utf-8")
    ...
    def test_classifies_safety_and_retries_without_following_symlink(self) -> None:
        ...
        self.assertEqual(records["capture.zip"]["classification"], "KEEP")
        self.assertEqual(records["capture.zip"]["reason"], "selected_result_evidence")
        self.assertEqual(records["unreferenced.zip"]["classification"], "DELETE_CANDIDATE")
        self.assertEqual(records["pointers/source.txt"]["classification"], "KEEP")
        self.assertEqual(records["pointers/source.txt"]["reason"], "source_pointer")
